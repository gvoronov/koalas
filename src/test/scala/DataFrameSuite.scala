...
class DataFrameSuite extends FunSuite {
  ...
  test("get numerical value series and sum it") {
    val atempSum1 = df[NumericalValue]("atemp").reduce((a, b) => a + b)
    assert(atempSum1 ~= 8268.4955)

    val atempSum2 = df[NumericalValue]("atemp").sum
  ...
  test("get numerical value series and get its mean") {
    val atempMeanTuple: Tuple2[NumericalValue, Int] =
      df[NumericalValue]("atemp")
      .map(atemp => (atemp, 1))
  ...
  test("partition dataframe and sum partioned column") {
    ...
    val cntLeftSum = leftDF[NumericalValue]("cnt").reduce((a, b) => a + b)
    val cntRightSum = rightDF[NumericalValue]("cnt").reduce((a, b) => a + b)
